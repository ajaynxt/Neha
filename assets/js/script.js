...
// Scroll-controlled cinematic videos. No autoplay is required.
...
function seekVideo(video, progress) {
  ...
  const target = clamp(progress) * Math.max(.1, video.duration - .05);
...
function updateHero() {
  if (reducedMotion || !heroSection || !heroVideo) return;
  const rect = heroSection.getBoundingClientRect();
  const scrollable = Math.max(1, heroSection.offsetHeight - window.innerHeight);
  const progress = clamp(-rect.top / scrollable);
  const targetTime = progress * Math.max(.1, heroDuration - .05);

  if (heroVideo.readyState >= 1 && Math.abs(heroVideo.currentTime - targetTime) > .025) {
...
function updatePairs() {
  ...
  pairSections.forEach(pair => {
    ...
    if (rect.bottom < -viewport * .4 || rect.top > viewport * 1.4) return;

    const scrollable = Math.max(1, pair.section.offsetHeight - viewport);
    const progress = clamp(-rect.top / scrollable);

    // Clip A owns the first half, clip B owns the second half.
    // Their overlap creates a smooth cinematic crossfade instead of two long sections.
    const localA = clamp(progress / .52);
    const localB = clamp((progress - .48) / .52);
    seekVideo(pair.videoA, localA);
    seekVideo(pair.videoB, localB);

    const blend = smoothstep(.43, .57, progress);
